...
"""
import os
os.environ["KERAS_BACKEND"] = "torch"  # must be set before any `import keras`, belt-and-braces with config.py
import sys
import pandas as pd
...
with tab_dashboard:
    ...
    if log_df.empty:
        ...
        # ---- Integrated Option B Fix: Timeline Model Usage Mapping ----
        st.markdown("**Model usage**")
        chart_data = log_df[['timestamp', 'model_used']].copy()
        chart_data['timestamp'] = pd.to_datetime(chart_data['timestamp'])
        
        # Group metrics to prevent raw index rendering issues
        chart_data = chart_data.groupby(['timestamp', 'model_used']).size().unstack(fill_value=0)
